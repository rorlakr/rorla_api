...
feature 'Answer management' do
  ...
  scenario '> 한 질문에 달린 답변 목록을 얻는다' do
    get '/api/questions/37/answers.json', nil, @header
  ...
  scenario '> 한 답변의 상세 정보를 얻는다' do
    get '/api/questions/37/answers/3701.json', nil, @header
    expect(json).to include({
      id: 3701,
      question_id: 37,
      user_id: 1004,
  ...
  scenario '> 질문에 대한 답변을 작성한다' do
    post '/api/questions/37/answers.json', {answer: {content: 'Do It Yourself'}}, @header
    expect(json).to include({
  ...
  scenario '> 답변을 수정한다' do
    put '/api/questions/37/answers/3701.json', {answer: {content: 'Hacked'}}, @header
    expect(json).to include({
      content: 'Hacked'
    }.with_indifferent_access)
  end
  ...
  scenario '> 답변을 삭제한다' do
    delete '/api/questions/37/answers/3701.json', nil, @header
    expect(json).to include({
      message: 'destroyed'
    }.with_indifferent_access)
  end
